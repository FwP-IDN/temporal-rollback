...
func (w *Workflow) OrderWorkflow(ctx workflow.Context, versionOverride map[string]int, order Order) error {
	...
	processOrderVersion := util.GetVersion(ctx, versionOverride, "OrderWorkflow:ProcessOrder", workflow.DefaultVersion, 2)

	switch processOrderVersion {
	case workflow.DefaultVersion:
		// Process the order with the default logic
	...
	default:
		return workflow.NewContinueAsNewError(ctx, w.OrderWorkflow, versionOverride, order)
	}
